...
public class table2 extends JFrame {
	 ...
	 * @throws Exception 
	 */
	public table2(ArrayList<MongoParameterPasser> list) throws Exception {
		setTitle("Table");
		...
		DefaultTableModel model = new DefaultTableModel(new String[]{"Patient Id","Patient Name", "Medicine"}, 0);
		...
		for(int i=0;i<sz;i++) {
			...
			nm=aes.decrypt(nm);
			String med=list.get(i).encrypted_meds;
			med=aes.decrypt(med);
			model.addRow(new Object[]{pid,nm,med});
			jt.setModel(model);
		}
